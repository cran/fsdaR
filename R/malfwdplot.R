...
#' @param ylim Control \code{y} scale in plot. Vector with two elements controlling
#'      minimum and maximum on the y axis. Default is to use automatic scale.
#'
#' @param xlim Control \code{x} scale in plot. Vector with two elements controlling
...
#'      matrix of MD. The default value of subsize is \code{(nrow(MAL) - ncol(MAL) + 1):nrow(MAL)}
#'
#' @param fg.thresh (alternative to fg.unit) numeric vector of length 1 or 2 which specifies the highlighted trajectories.
...
#' @param fg.cex Controls the font size of the labels of the trajectories in foreground.
#' @param bg.thresh Numeric vector of length 1 or 2 which specifies how to define the unimmportant trajectories.
...
#'    had a residual greater than bg.thresh(1) and smaller than bg.thresh(2). The default is:
...
#'    MD trajectory in order to have information about the corresponding unit, the associated
